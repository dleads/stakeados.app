...
export async function generateMetadata({
  ...
  if (error || !article) {
    return {
      title: 'Artículo No Encontrado | Stakeados',
      description: 'El artículo de noticias solicitado no pudo ser encontrado.',
    };
  }
  ...
  const title =
    typeof titleObj === 'string'
      ? titleObj
      : titleObj[locale] || titleObj['en'] || 'Artículo Sin Título';

  const summary =
    typeof summaryObj === 'string'
      ? summaryObj
      : summaryObj[locale] || summaryObj['en'] || 'Sin resumen disponible';
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://stakeados.com';

  return {
    title: `${title} | Noticias Stakeados`,
    description: summary,
    openGraph: {
      title,
      description: summary,
      ...
      //   }
      // ] : undefined,
      url: `${baseUrl}/${locale}/noticias/${id}`,
    },
    twitter: {
    ...
    alternates: {
      canonical: `${baseUrl}/${locale}/noticias/${id}`,
      languages: {
        en: `${baseUrl}/en/news/${id}`,
        es: `${baseUrl}/es/noticias/${id}`,
      },
    },
  };
}
